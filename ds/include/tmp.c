...
static int BinarySearchRecursiveWrapper(int arr[], size_t left, size_t right, int key);
...
int BinarySearchIterative(int arr[], size_t left, size_t right, int key)
{
    size_t mid = 0;
...
int BinarySearchRecursive(int arr[], size_t left, size_t right, int key)
{
    assert(arr != NULL);
    
    return BinarySearchRecursiveWrapper(arr, left, right, key);
}
...
static int BinarySearchRecursiveWrapper(int arr[], size_t left, size_t right, int key)
{
    ...
    if (arr[mid] < key)
    {
        return BinarySearchRecursiveWrapper(arr, mid + 1, right, key);
    }
    ...
    return BinarySearchRecursiveWrapper(arr, left, mid - 1, key);
}
